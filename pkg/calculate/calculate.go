...
// Создаем стуктуру со стеком для чисел
...
func Calc(expression string) (float64, error) {
	// Создаем 2 стека, одни для чисел, другой для операций
	...
	for _, token := range tokens {
		...
		if token == "+" || token == "-" || token == "*" || token == "/" || token == "(" || token == ")" {
			// Проверяем есть пустой ли стек с операциями
			if len(Operations.stackOper) == 0 {
				Operations.Append(token)
			} else {

				// Если токен является открывающей скобкой то мы должны его просто добавить в стек с операциями, чтобы после он был для на с сигналом
				if token == "(" {
					Operations.Append(token)
					continue
				}

				// Если токен является закрывающей скобкой то нам нужно сосчитать всё что находится между до открывающей скобки
				if token == ")" {
					for {
						topOperation := Operations.Top()
						// Если в стеке дошли до открывающей скобки, то мы должны закончить цикл
						if topOperation == "(" {
							_ = Operations.Pop()
							break
						}
			...
			// Если токен не являетсся математической операцией, то мы записываем его в стек чисел
...
func GetTokens(expression string) []string {
	...
	for _, r := range expression {
		...
		// Проверяем является ли символ строки знаком математического вывражения
		if sym == "+" || sym == "-" || sym == "*" || sym == "/" || sym == "(" || sym == ")" {
			// Если да, то проверяем есть значение в переменной num. Если есть то добавляем его в массив
			if num != "" {
				result = append(result, num)
				// После добавления обновляем переменную num
				num = ""
...
// Функция считает простейшие математичческие операции
