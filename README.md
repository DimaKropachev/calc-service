...
0. Прежде чем устанавливать проект убедитесь, что у вас установлен **golang**. Если нет, то скачайте его перейдя по <a href="https://go.dev/dl/">ссылке</a>
...
3. Запустите сервер:
    ...
    go run ./cmd/main.go
    ```
4. После запуска сервис будет доступе по адресу:
...
- Сначала пишется `curl`. **curl** - это командная утилита, позволяющая отправлять запросы к веб-серверам и получить ответы.
